...
### 비주얼 스타일 프리셋 (--style)
입력 모드와는 별개로 11종의 고유 비주얼 스타일 프리셋을 지원합니다:
`anime_3d`(3D 애니 필름), `claymation`(클레이 애니메이션), `lofi_3d_figure`(로파이 3D 피규어), `stickman_sketch`(스틱맨 노트북 낙서), `asmr_cinematic`(ASMR 시네마틱), `neo_closure_vlog`(네오 클로저 브이로그), `retro_8bit`(레트로 8비트), `cyberpunk_neon`(사이버펑크 네온), `minimal_beauty_cf`(미니멀 뷰티 CF), `vlog_illus_self`(브이로그 일러스트 셀프), `wabi_sabi_japan`(와비사비 재팬 리얼)

### 스타일 잠금 (Style Lock) 및 캐릭터 잠금 (Character Lock)
- 설정한 비주얼 스타일에 따라 모든 컷에 특화 긍정형 프롬프트(Prefix)와 부정형 프롬프트(Negative)가 일괄 고정 적용됩니다.
- 캐릭터/브랜드 잠금 프롬프트를 입력하면, 해당 속성 프롬프트가 모든 컷의 스타일 바로 뒤에 자동으로 잠금 처리(Prepend)되어 컷 전반의 비주얼 일관성을 확보합니다.
...
## WanGP 플러그인 (Plugin)
본 프로그램은 WanGP의 플러그인 아키텍처를 지원하여, WanGP 내부에서 직접 플러그인 탭으로 로드해 사용할 수 있습니다.

- **위치:** `plugins/local_video_factory_plugin/`
- **구조:** `plugin.py`가 `WAN2GPPlugin`을 상속받아 WanGP UI 구성 단계에서 동적으로 연동됩니다.
- **기능:** 대본 입력, 스타일 프리셋/캐릭터 잠금 설정, 컷 분할 및 프롬프트 생성을 플러그인 탭 안에서 처리한 뒤, 원하는 컷의 **"🎯 WanGP 프롬프트에 주입"** 버튼을 누르면 WanGP 메인 탭의 긍정/부정 프롬프트 창으로 값이 즉시 입력됩니다.

## 다음 단계 (선택)
- Electron + FastAPI 데스크톱 앱
- shot_planner 튜닝(컷당 1문장 우선)으로 나레이션-영상 길이 싱크 개선
- 플러그인 고도화: `insert_after` 및 `request_global`을 이용한 비디오 자동 생성 트리거링 및 상태 감시 구현
